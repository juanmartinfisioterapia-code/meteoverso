...
const WMO = {
  ...
  51:{icon:"🌦️",label:"Llovizna ligera"},53:{icon:"🌧️",label:"Llovizna"},
  55:{icon:"🌧️",label:"Llovizna densa"},61:{icon:"🌧️",label:"Lluvia ligera"},
  63:{icon:"🌧️",label:"Lluvia moderada"},65:{icon:"🌧️",label:"Lluvia fuerte"},
...
async function fetchWeather(lat, lon, param) {
  const url =
    ...
    `weather_code,wind_speed_10m,wind_direction_10m,precipitation,surface_pressure` +
    `&wind_speed_unit=kmh&timezone=auto&models=${param}`;
...
function buildConsensus(data) {
  ...
  return {
    ...
    wind:avg("wind"), precip:avg("precip"),
...
export default function App() {
  ...
  const runModels = async (lat, lon, name) => {
    ...
    if (!anyOk) setErrMsg("No se pudieron cargar los datos. Verifica tu conexión.");
  };
  ...
  const doSearch = async () => {
    ...
    try {
      ...
      if (!cities.length) { setStatus("error"); setErrMsg(`No encontré "${q}". Prueba con otra ciudad.`); return; }
      const c = cities[0];
      setInput([c.name, c.admin1, c.country].filter(Boolean).join(", "));
      runModels(c.latitude, c.longitude, c.name);
    } catch(e) { setStatus("error"); setErrMsg("Error de conexión: " + e.message); }
  };
  ...
  return (
    <div style={{minHeight:"100vh",background:"#06101e",color:"#e2e8f0",fontFamily:"'DM Sans',system-ui,sans-serif",overflowX:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1,maxWidth:900,margin:"0 auto",padding:"32px 16px 80px"}}>

        {/* HEADER */}
        <div style={{textAlign:"center",marginBottom:32,animation:"fadeUp .5s ease both"}}>
          <div style={{fontSize:36,marginBottom:8,display:"inline-block",animation:"float 5s ease-in-out infinite"}}>🌤️</div>
          <h1 style={{fontFamily:"'Syne',sans-serif",fontSize:"clamp(32px,7vw,52px)",fontWeight:900,letterSpacing:"-.03em",lineHeight:1,marginBottom:6,background:"linear-gradient(135deg,#bae6fd 0%,#38BDF8 50%,#60A5FA 100%)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>
            Meteoverso
          </h1>
          <div style={{color:"#164e63",fontSize:10,letterSpacing:".14em",textTransform:"uppercase",fontFamily:"'DM Mono',monospace",marginBottom:8}}>
            comparador meteorológico científico
          </div>
          <p style={{color:"#1e4060",fontSize:13,lineHeight:1.6}}>
            Tres modelos · Un consenso · Sin registro
          </p>
        </div>

        {/* SEARCH */}
        <div style={{position:"relative",maxWidth:560,margin:"0 auto 24px",animation:"fadeUp .5s ease .08s both"}}>
          ...
          >
            ...
            <button onClick={doSearch} disabled={isLoading}
              style={{background:isLoading?"#0c2a42":"linear-gradient(135deg,#0ea5e9,#3b82f6)",color:isLoading?"#1e4060":"#fff",border:"none",borderRadius:10,padding:"8px 20px",fontSize:13,fontWeight:700,cursor:isLoading?"not-allowed":"pointer",fontFamily:"'DM Sans',sans-serif",flexShrink:0,opacity:isLoading?.5:1,whiteSpace:"nowrap",transition:"all .15s"}}>
        ...
        {/* Error */}
        {status==="error" && errMsg && (
          <div style={{maxWidth:560,margin:"0 auto 20px",background:"rgba(252,165,165,.07)",border:"1px solid rgba(252,165,165,.25)",borderRadius:10,padding:"12px 16px",color:"#FCA5A5",fontSize:13,animation:"fadeUp .3s ease both"}}>
            ⚠️ {errMsg}
          </div>
        )}
        ...
        {status==="idle" && (
          <div style={{animation:"fadeUp .5s ease .12s both"}}>
            ...
            <div style={{textAlign:"center"}}>
              <p style={{color:"#0f2035",fontSize:12,lineHeight:1.9}}>Sin registro · Sin API key · Datos científicos reales</p>
            </div>
          </div>
        )}
          ...
          <div style={{animation:"fadeUp .4s ease both"}}>
            ...
            {loc && (
              <div style={{textAlign:"center",marginBottom:20}}>
                <div style={{color:"#0f2a42",fontSize:10,textTransform:"uppercase",letterSpacing:".14em",fontFamily:"'DM Mono',monospace",marginBottom:4}}>comparando para</div>
                <div style={{fontFamily:"'Syne',sans-serif",fontSize:26,fontWeight:900,color:"#f0f9ff"}}>{loc.name}</div>
                <div style={{color:"#0f2035",fontSize:10,fontFamily:"'DM Mono',monospace",marginTop:3}}>{loc.lat?.toFixed(3)}°N · {loc.lon?.toFixed(3)}°E</div>
              </div>
            )}

            {/* Consensus */}
            <div style={{background:"rgba(56,189,248,.04)",border:"1px solid rgba(56,189,248,.14)",borderRadius:18,padding:"22px",marginBottom:14}}>
              {isLoading && !con ? (
                <div style={{display:"flex",gap:20,flexWrap:"wrap",alignItems:"center"}}>
                  <div style={{height:70,width:180,background:"rgba(255,255,255,.04)",borderRadius:10,animation:"shimmer 1.4s ease infinite"}}/>
                ...
                <div style={{display:"flex",gap:20,flexWrap:"wrap",alignItems:"center"}}>
                  <div>
                    <div style={{color:"#0f2a42",fontSize:9,textTransform:"uppercase",letterSpacing:".12em",fontFamily:"'DM Mono',monospace",marginBottom:6}}>Consenso · 3 modelos</div>
                    <div style={{display:"flex",alignItems:"flex-end",gap:8,marginBottom:3}}>
                      <span style={{fontSize:50}}>{con.info.icon}</span>
                      <span style={{fontFamily:"'Syne',sans-serif",fontSize:56,fontWeight:900,color:"#f0f9ff",lineHeight:1}}>{con.temp}°C</span>
                    </div>
                    <div style={{color:"#2e6b8a",fontSize:13}}>{con.info.label}</div>
                  </div>
                  <div style={{flex:1,minWidth:180}}>
                    <div style={{marginBottom:12}}>
                      ...
                      <div style={{color:"#0f2035",fontSize:10,marginTop:4,fontFamily:"'DM Mono',monospace"}}>
                    ...
                    <div style={{display:"flex",gap:14,flexWrap:"wrap"}}>
                      {[{l:"Sensación",v:`${con.feels}°C`},{l:"Humedad",v:`${con.humidity}%`},{l:"Viento",v:`${con.wind}km/h`},{l:"Precip.",v:`${con.precip}mm`}].map(({l,v})=>(
                        <div key={l}>
                          ...
                          <div style={{color:"#e0f2fe",fontSize:14,fontWeight:700,fontFamily:"'DM Mono',monospace"}}>{v}</div>
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <div style={{display:"flex",gap:12,flexWrap:"wrap",marginBottom:14}}>
              {MODELS.map(m=>{
                const d=data[m.id];
                return(
                  <div key={m.id} className="src-card" style={{flex:"1 1 200px",minWidth:180,background:m.bg,border:`1px solid ${m.br}`,borderRadius:16,padding:"18px",position:"relative"}}>
                    <div style={{position:"absolute",top:11,right:11,background:`${m.badgeC}18`,border:`1px solid ${m.badgeC}35`,borderRadius:20,padding:"2px 7px",fontSize:8,color:m.badgeC,fontFamily:"'DM Mono',monospace",fontWeight:700}}>{m.badge}</div>
                    <div style={{display:"flex",alignItems:"center",gap:7,marginBottom:14}}>
                      <span style={{background:`${m.color}20`,border:`1px solid ${m.br}`,borderRadius:8,padding:"3px 10px",color:m.color,fontSize:12,fontWeight:700,fontFamily:"'DM Mono',monospace"}}>{m.name}</span>
                      <span style={{color:"#1e3a5f",fontSize:10}}>{m.tag}</span>
                    </div>
                    {isLoading?(
                      [52,38,68,55,60].map((w,i)=><div key={i} style={{height:10,width:`${w}%`,background:"rgba(255,255,255,.05)",borderRadius:5,marginBottom:8,animation:"shimmer 1.4s ease infinite",animationDelay:`${i*.1}s`}}/>)
                    ):!d?(
                      <div style={{color:"#1e3a5f",fontSize:12}}>Sin datos</div>
                    ):d.error?(
                      <div style={{color:"#FCA5A5",fontSize:11}}>⚠️ {d.error}</div>
                    ):(
                      <>
                        <div style={{display:"flex",alignItems:"flex-end",gap:7,marginBottom:3}}>
                          <span style={{fontSize:40,lineHeight:1}}>{d.info.icon}</span>
                          <span style={{fontFamily:"'Syne',sans-serif",fontSize:44,fontWeight:900,color:"#f0f9ff",lineHeight:1}}>{d.temp}°</span>
                        </div>
                        <div style={{color:m.color,fontSize:12,fontWeight:600,marginBottom:14}}>{d.info.label}</div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"7px 10px"}}>
                          {[{l:"Sensación",v:`${d.feels}°`},{l:"Humedad",v:`${d.humidity}%`},{l:"Viento",v:`${d.wind}km/h ${windDir(d.windD)}`},{l:"Precip.",v:`${d.precip}mm`},{l:"Presión",v:`${d.pressure}hPa`}].map(({l,v})=>(
                            <div key={l}>
                    ...
                    <div style={{position:"absolute",bottom:10,right:12,color:"#0f2035",fontSize:9,fontFamily:"'DM Mono',monospace"}}>{m.res}</div>
                  </div>
                );
              })}
            </div>
            ...
            {status==="done" && con && (
              <div style={{background:"rgba(255,255,255,.02)",border:"1px solid rgba(56,189,248,.1)",borderRadius:12,overflow:"hidden",marginBottom:14}}>
                <div style={{padding:"11px 18px",borderBottom:"1px solid rgba(255,255,255,.04)"}}>
                  ...
                  <table style={{width:"100%",borderCollapse:"collapse",minWidth:360}}>
                    <thead>
                      <tr style={{background:"rgba(255,255,255,.01)"}}>
                        <td style={{padding:"8px 18px",color:"#0f2035",fontSize:9,textTransform:"uppercase",letterSpacing:".07em",fontFamily:"'DM Mono',monospace"}}>Parámetro</td>
                        {MODELS.map(m=>(
                    ...
                    <tbody>
                      {[{k:"temp",l:"Temp.",u:"°C"},{k:"feels",l:"Sensación",u:"°C"},{k:"humidity",l:"Humedad",u:"%"},{k:"wind",l:"Viento",u:"km/h"},{k:"precip",l:"Precip.",u:"mm"},{k:"pressure",l:"Presión",u:"hPa"}].map(({k,l,u},ri)=>{
                        ...
                        return(
                          <tr key={k} style={{background:ri%2?"rgba(255,255,255,.01)":"transparent",borderTop:"1px solid rgba(255,255,255,.03)"}}>
                            <td style={{padding:"9px 18px",color:"#2e6b8a",fontSize:12}}>{l}</td>
                            {MODELS.map(m=>{
                              const v=data[m.id]?.[k];
                              const hi=v!=null&&v===mx&&mn!==mx,lo=v!=null&&v===mn&&mn!==mx;
                              return(
                                <td key={m.id} style={{padding:"9px 8px",textAlign:"center"}}>
            ...
            {/* Science note */}
            {status==="done" && (
              <div style={{background:"rgba(56,189,248,.03)",border:"1px solid rgba(56,189,248,.1)",borderRadius:12,padding:"16px 18px"}}>
                <div style={{color:"#38BDF8",fontSize:10,textTransform:"uppercase",letterSpacing:".1em",fontFamily:"'DM Mono',monospace",marginBottom:10}}>🔬 Sobre los modelos</div>
                {[
                  {id:"best", t:"Mejor Modelo", d:"Selecciona automáticamente el mejor modelo para España, integrando datos de estaciones AEMET vía Open-Meteo."},
                  {id:"ecmwf",t:"ECMWF",        d:"Motor detrás de El Tiempo.es y Meteored. Modelo global más preciso para 3–10 días."},
                  {id:"icon", t:"ICON-EU",      d:"Modelo alemán DWD. Mayor resolución pública disponible (2 km). Muy preciso para Europa."},
                ].map(({id,t,d})=>{
                  const m=MODELS.find(x=>x.id===id);
                  return(
                    <div key={id} style={{display:"flex",gap:8,alignItems:"flex-start",marginBottom:9}}>
                      <span style={{background:`${m.color}20`,color:m.color,fontSize:9,fontWeight:700,padding:"2px 9px",borderRadius:7,flexShrink:0,fontFamily:"'DM Mono',monospace",border:`1px solid ${m.br}`,marginTop:1}}>{t}</span>
                      <span style={{color:"#1e4060",fontSize:12,lineHeight:1.6}}>{d}</span>
                    </div>
                  );
                })}
              </div>
